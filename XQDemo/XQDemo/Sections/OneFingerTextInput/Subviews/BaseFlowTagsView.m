...
@implementation BaseFlowTagsView

+ (void)setActiveBaseFlowTagsView:(BaseFlowTagsView*)view
{
    static BaseFlowTagsView *activeView = nil;
    if(view != activeView){
...
{
    self = [super initWithFrame:CGRectMake(0, 0, image.size.width+kFrameGap, image.size.height+kFrameGap)];
    if(self){
        ...
        _deleteButton.frame = CGRectMake(0, 0, kFrameGap, kFrameGap);
        _deleteButton.center = _imageView.frame.origin;
        [_deleteButton addTarget:self action:@selector(pushedDeleteBtn:) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:_deleteButton];
        
        _circleView = [[CLCircleView alloc] initWithFrame:CGRectMake(0, 0, kFrameGap, kFrameGap)];
...
{
    BaseFlowTagsView *nextTarget = nil;
    
    const NSInteger index = [self.superview.subviews indexOfObject:self];
    
    for(NSInteger i=index+1; i<self.superview.subviews.count; ++i){
        UIView *view = [self.superview.subviews objectAtIndex:i];
        if([view isKindOfClass:[BaseFlowTagsView class]]){
            nextTarget = (BaseFlowTagsView*)view;
            break;
        }
    }
    ...
    [[self class] setActiveBaseFlowTagsView:nextTarget];
...
- (void)setScale:(CGFloat)scale
{
    ...
    rct.origin.x += (rct.size.width - (_imageView.width + kFrameGap)) / 2;
    rct.origin.y += (rct.size.height - (_imageView.height + kFrameGap)) / 2;
    rct.size.width  = _imageView.width + kFrameGap;
    rct.size.height = _imageView.height + kFrameGap;
    self.frame = rct;
...
{
    [[self class] setActiveBaseFlowTagsView:self];
}
